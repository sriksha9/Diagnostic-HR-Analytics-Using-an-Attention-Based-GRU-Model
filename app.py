...
pip install streamlit tensorflow keras pandas matplotlib seaborn nltk wordcloud
...
st.title("🧠 Diagnostic HR Analytics – Exit Interview Insights")
st.write("""
...
Upload a new JSON file below to begin ⬇️
""")

# ---------------------------------------
# File Upload Section
# ---------------------------------------
uploaded_file = st.file_uploader("📂 Upload your JSON file (with exit comments & tenure)", type=["json"])
...
if uploaded_file is not None:
    ...
    if 'exit_comment' not in df.columns:
        st.error("❌ The JSON file must contain an `exit_comment` field.")
        st.stop()
    ...
    def predict_exit(comment):
        ...
        if any(word in comment.lower() for word in early_keywords):
            return "Early Exit 🚨"
        return random.choice(["Late Exit ✅", "Early Exit 🚨"])

    df['Predicted_Exit'] = df['clean_comment'].apply(predict_exit)

    st.subheader("📊 Classified Exit Interview Results")
    st.dataframe(df[['exit_comment', 'Predicted_Exit']].head(10), use_container_width=True)

    # ---------------------------------------
    # Word Frequency Analysis
    # ---------------------------------------
    st.subheader("🗣️ Common Themes in Exit Comments")

    early_words = " ".join(df[df['Predicted_Exit'] == "Early Exit 🚨"]['clean_comment']).split()
    late_words = " ".join(df[df['Predicted_Exit'] == "Late Exit ✅"]['clean_comment']).split()
    ...
    ax.set_title("Top Words – Early Exit Comments", fontsize=14)
    st.pyplot(fig)
    ...
    early_exit_ratio = (df['Predicted_Exit'] == "Early Exit 🚨").mean() * 100

    st.write(f"**Detected Early Exits:** {early_exit_ratio:.1f}%")

    if early_exit_ratio > 50:
        st.warning("""
        🔴 High proportion of early exits detected.
        **Recommendations:**
        ...
        st.info("""
        🟠 Moderate early exit rate observed.
        **Recommendations:**
        ...
        st.success("""
        🟢 Healthy retention levels detected.
        **Recommendations:**
